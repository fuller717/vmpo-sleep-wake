...
mouse = 'hdc222'
fig_title = mouse + ': 1pm CNO injection'
path = 'C:/Users/avenner/Desktop/Python Analysis/sleep_wake_analysis/hdc m3_cage change/results/'  
...
for d in np.arange(stamp1.size):
    timestamp1 = np.append(timestamp1, dt.datetime.strptime(stamp_lis1[d], "%d/%m/%Y %H:%M:%S"))
    timestamp2 = np.append(timestamp2, dt.datetime.strptime(stamp_lis2[d], "%d/%m/%Y %H:%M:%S"))
